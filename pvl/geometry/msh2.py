...
class Msh2TetraSummary(FrozenModel):
    ...
    surface_physical_names: dict[int, str]
    tetrahedra_by_physical_tag: dict[int, int]
    triangles_by_physical_tag: dict[int, int]
    ...
    @property
    def triangles_by_physical_name(self) -> dict[str, int]:
        return {
            self.surface_physical_names[tag]: count
            for tag, count in self.triangles_by_physical_tag.items()
            if tag in self.surface_physical_names
        }
...
def _parse_physical_names(lines: list[str]) -> dict[int, dict[int, str]]:
    ...
    for row in section[1:]:
        ...
        result.setdefault(dimension, {})[tag] = name[1:-1]
        parsed += 1
    if parsed != expected:
        raise ValueError(f"MSH2 physical-name count mismatch: expected {expected}, parsed {parsed}")
    return result
...
def parse_msh2_tetra_summary(path: Path) -> Msh2TetraSummary:
    ...
    if not mesh_format or not mesh_format[0].startswith("2.2 "):
        raise ValueError("PVL parser requires ASCII MSH 2.2")

    names_by_dimension = _parse_physical_names(lines)
    physical_names = names_by_dimension.get(3, {})
    surface_physical_names = names_by_dimension.get(2, {})
    ...
    for row in section[1:]:
        ...
        if element_type == 2:
            if number_of_tags < 1 or len(values) != first_node + 3:
                raise ValueError(f"invalid triangle MSH2 row: {row}")
            physical_tag = int(values[3])
            triangle_counts[physical_tag] = triangle_counts.get(physical_tag, 0) + 1
            continue
        if element_type != 4:
            continue
    ...
    return Msh2TetraSummary(
        ...
        triangles_by_physical_tag=triangle_counts,
